...
| `core/PowerSetup.kt`、`MainActivity.kt` | readiness 必须满足无障碍、悬浮窗、密钥 | 按 Xposed/兼容模式判定；区分读取可用与模型配置完成 |
...
现有 touched files 预计包括 `settings.gradle.kts`、根/模块 Gradle、Manifest、`ChatCaptureService.kt`、`ChatModels.kt`、`Prefs.kt`、`PowerSetup.kt`、`MainActivity.kt`、`SettingsActivity.kt`、`OverlayController.kt`、`ContextBuilder.kt` 及对应测试；Jev 模型输入序列化需携带群成员/引用等语义。最终实现按实际职责拆分，不机械追求上述文件数量。
